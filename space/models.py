...
class ScreenshotRequest(BaseModel):
    ...
    device: Optional[str] = Field(
        None,
        description="The device to use for the screenshot (e.g. 'iPhone 12 Pro Max', 'Samsung Galaxy S21')",
        example="desktop",
    )
    full_page: Optional[bool] = Field(
        True, description="Whether to take a full-page screenshot", example=False
    )
    color_scheme: Optional[Literal["dark", "light"]] = Field(
        "light",
        description="The color scheme to use for the screenshot (e.g. 'dark', 'light')",
        example="dark",
    )
    locator: Optional[str] = Field(
        None,
        description="The locator to use for the screenshot (e.g. '#my-element')",
        example="#my-element",
    )
...
class PdfRequest(BaseModel):
    ...
    format: Optional[str] = Field(
        "A4",
        description="The format of the PDF (e.g. 'A4', 'Letter', 'Legal')",
        example="A4",
    )
    ...
    def validate_format(cls, value):
        if value not in pdf_format_list:
            raise ValueError(
                f"Invalid format. Supported formats are {pdf_format_list}."
            )
...
class ScrapeRequest(BaseModel):
    ...
    locator: Optional[str] = Field(
        None,
        description="The locator to use for the scraping (e.g. '#my-element')",
        example="#my-element",
    )
    user_agent: Optional[str] = Field(
        None,
        description="The user agent to use for the scraping",
